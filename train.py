...
from environments.frozenlake_env import FrozenLakeEnvironment
...
class TinyZeroTrainer:
    ...
    def setup_models(self):
        ...
        if self.config['distributed']['use_fsdp']:
            ...
            self.policy_model = setup_fsdp_model(
                self.policy_model, fsdp_config)
            self.reference_model = setup_fsdp_model(
                self.reference_model, fsdp_config)
        else:
    ...
    def setup_environments(self):
        ...
        self.multiplication_env = MultiplicationEnvironment(
            env_config['multiplication'])
        # FrozenLake is optional and requires gym
        if 'frozenlake' in env_config:
            try:
                self.frozenlake_env = FrozenLakeEnvironment(
                    env_config['frozenlake'])
            except Exception:
                self.frozenlake_env = None
        else:
            self.frozenlake_env = None
        ...
        if getattr(self, 'frozenlake_env', None) is not None:
            self.environments['frozenlake'] = self.frozenlake_env

    def setup_algorithm(self):
        """Initialize A*PO algorithm."""
        # Allow algorithm selection via config: 'astarpo' (default) or 'ragen'
        astarpo_config = AStarPOConfig(**self.config['astarpo'])
        astarpo = AStarPO(astarpo_config, self.device)

        alg_name = self.config.get('algorithm', {}).get(
            'name', 'astarpo').lower()
        if alg_name == 'ragen':
            try:
                from algorithms.ragen_astarpo import RAGEN
                # RAGEN wraps A*PO helpers internally
                self.algorithm = RAGEN(self.config['astarpo'], self.device)
            except Exception:
                # Fallback to A*PO if RAGEN import fails
                self.algorithm = astarpo
        else:
            self.algorithm = astarpo
    ...
    def setup_optimizer(self):
        ...
        # Get learning rate - ensure it's a float
        if 'learning_rate' in training_config:
            lr = float(training_config['learning_rate'])
        elif 'learning_rate' in self.config['astarpo']:
            lr = float(self.config['astarpo']['learning_rate'])
        else:
            lr = 1e-5  # Default fallback
        
        # Get weight decay - ensure it's a float
    ...
    def generate_action(self, state: str):
        ...
        try:
            ...
            total_log_prob = torch.stack(
                log_probs).sum() if log_probs else torch.tensor(0.0)
    ...
    def train_iteration(self):
        ...
        # Perform algorithm update (A*PO or RAGEN)
        update_stats = self.algorithm.update_policy(
            self.policy_model,
    ...
    def evaluate_model(self) -> dict:
        ...
        if self.logger:
            self.logger.logger.info(
                f"Evaluation at iteration {self.iteration}: {eval_stats}")
    ...
    def train(self):
        ...
        try:
            for iteration in range(total_iterations):
                ...
                if is_main_process():
                    print(f"Iteration {self.iteration}: reward={stats.get('avg_trajectory_reward', 0):.3f}")
